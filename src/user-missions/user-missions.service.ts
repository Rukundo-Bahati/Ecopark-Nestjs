...
import { CreateUserMissionDto, UserMissionStatus } from './dto/create-user-mission.dto';
...
export class UserMissionsService {
  ...
  async findOne(id: string, userId: string) {
    ...
    if (!userMission || userMission.userId !== userId) throw new NotFoundException('User mission not found');
    return userMission;
  }
  ...
  async verify(id: string, verifierId: string) {
    ...
    return this.prisma.userMission.update({
      where: { id },
      data: { status: UserMissionStatus.VERIFIED, verifiedAt: new Date(), verifierId },
    });
  }
}
